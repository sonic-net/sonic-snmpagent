...
class InterfaceMIBUpdater(MIBUpdater):
    ...
    def get_speed(self, sub_id):
        """
        :param sub_id: The 1-based sub-identifier query.
        :return: speed value for the respective sub_id.
        """
        entry = self._get_if_entry(sub_id)
        if not entry:
            return

        return int(entry.get(b"speed", 0))
...
class InterfaceMIBObjects(metaclass=MIBMeta, prefix='.1.3.6.1.2.1.31.1'):
    ...
    ifHighSpeed = SubtreeMIBEntry('1.1.15', if_updater, ValueType.GAUGE_32, if_updater.get_speed)
