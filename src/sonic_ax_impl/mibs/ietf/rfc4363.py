...
class Dot1qVlanCurrentUpdater(MIBUpdater):
    ...
    def update_data(self):
        ...
        vlan_entries = Namespace.dbs_keys(self.db_conn, mibs.CONFIG_DB, "VLAN|*")
        if not vlan_entries:
            return
        ...
        for vmem_entry in vlanmem_entries:
            ...
            elif 'PortChannel' in port_name:
                port = port_name[len('PortChannel'):]
            else:
                continue

            port = int(port)
            tag_type = self.db_conn[0].get(mibs.CONFIG_DB, vmem_entry, 'tagging_mode')
            if tag_type:
            ...
            if_index = mibs.get_index_from_str(port_name)
            if tag_type == 'untagged':
