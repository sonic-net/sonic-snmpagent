...
#
# ./sysObjectId_pass.py -g .1.3.6.1.2.1.1.2.0
# ./sysObjectId_pass.py -n .1.3.6.1.2.1.1.2
#
...
#
# # snmpget  -v2c -cpublic localhost .1.3.6.1.2.1.1.2
...
# this is the one oid
myoid = '.1.3.6.1.2.1.1.2.0'
# and the version without the .0
myoidsub1 = '.1.3.6.1.2.1.1.2'
...
result = re.search('HwSku: (.*) - Distribution', sysDescription)
mibObj = result.group(1)
mibObj = mibObj[0].lower() + mibObj[1:]
cmd = "/usr/bin/snmptranslate -m /usr/share/snmp/mibs/BROADCOM-SONIC-PRODUCT-MIB.txt -M /usr/share/snmp/mibs -IR -On " + mibObj
proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True, stderr=subprocess.STDOUT)
oid = proc.communicate()[0]
#oid = ".1.3.6.1.4.1.4413.1.2.2.9"
print("%s\nOBJECTID\n%s" % (myoid, oid))
